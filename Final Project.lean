...
theorem chineseRemainder_k_equations
  ...
  -- First establish that d divides the product
        have h_d_div_prod : d ∣ ∏ j in Finset.univ.erase i, n j := by
          have h_Ni_eq : Ni i = ∏ j in Finset.univ.erase i, n j := by
            show (∏ j : Fin k, n j) / n i = ∏ j in Finset.univ.erase i, n j
            have h_prod_split : ∏ j : Fin k, n j = n i * ∏ j in Finset.univ.erase i, n j :=
              (Finset.mul_prod_erase Finset.univ (fun j => n j) (Finset.mem_univ i)).symm
            rw [h_prod_split]
            exact Nat.mul_div_cancel_left _ (hn i)
          rw [← h_Ni_eq]
          exact hd_Ni

        -- Convert Nat.Prime to Prime
        have hd_prime' : Prime d := Nat.Prime.prime hd_prime

        -- Use generalized induction tracking that we're on Finset.univ.erase i
        classical
        suffices ∀ (s : Finset (Fin k)), s ⊆ Finset.univ.erase i → d ∣ ∏ j in s, n j → ∃ j ∈ s, d ∣ n j by
          obtain ⟨j, hj_mem, hj_dvd⟩ := this (Finset.univ.erase i) (le_refl _) h_d_div_prod
          use j
          exact ⟨Finset.ne_of_mem_erase hj_mem, hj_dvd⟩

        intro s hs h_dvd
        refine Finset.induction_on s ?_ ?_ h_dvd
        · intro h
          exfalso
          rw [Finset.prod_empty] at h
          exact Nat.Prime.not_dvd_one hd_prime h
        · intro a t ha ih h_dvd
          rw [Finset.prod_insert ha] at h_dvd
          cases hd_prime'.dvd_mul.mp h_dvd with
          | inl h_dvd_a =>
            use a
            exact ⟨Finset.mem_insert_self a t, h_dvd_a⟩
          | inr h_dvd_t =>
            obtain ⟨j, hj_mem, hj_dvd⟩ := ih h_dvd_t
            use j
            exact ⟨Finset.mem_insert_of_mem hj_mem, hj_dvd⟩
    ...
    have h_eq : (Ni i : ℤ) * x i = -(Int.gcdB (Ni i : ℤ) (n i : ℤ) * (n i : ℤ)) + 1 := by
      linarith
    rw [h_eq]
    have : -(Int.gcdB ↑(Ni i) ↑(n i) * ↑(n i)) + 1 ≡ 1 [ZMOD ↑(n i)] := by
      have h_zero : Int.gcdB ↑(Ni i) ↑(n i) * ↑(n i) ≡ 0 [ZMOD ↑(n i)] := by
        apply Int.modEq_zero_iff_dvd.mpr
        exact dvd_mul_left (↑(n i)) (Int.gcdB ↑(Ni i) ↑(n i))
      calc -(Int.gcdB ↑(Ni i) ↑(n i) * ↑(n i)) + 1
          ≡ -0 + 1 [ZMOD ↑(n i)] := by
            apply Int.ModEq.add_right
            exact Int.ModEq.neg h_zero
        _ ≡ 1 [ZMOD ↑(n i)] := by norm_num
    exact this
